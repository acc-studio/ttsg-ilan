...
function dataDir() {
    const base = process.env.TTSG_DATA_DIR && process.env.TTSG_DATA_DIR.trim().length > 0
...
async function ensureContext(headless = HEADLESS_DEFAULT) {
    ...
    if (context && contextHeadless === headless)
        return context;
    if (context) {
...
async function getPage(headless) {
 ...
 */
async function isLoggedIn(page, waitMs = 8000) {
 ...
 * `windowless`.
 */
export async function login(timeoutMs = 300_000) {
    ...
    if (!(await isLoggedIn(page))) {
        ...
        if (!(await isLoggedIn(page))) {
            throw new Error("Timed out waiting for login. Complete the e-Devlet/member login in the browser window, then retry.");
        }
    }
    // If normal ops stay headed, we're done — keep the (now logged-in) window.
    if (!HEADLESS_DEFAULT)
        return { status: "logged_in", windowless: false };
...
export async function loadOffices() {
    ...
    const offices = await select.locator("option").evaluateAll((opts) => opts
        .map((o) => ({ name: (o.textContent || "").trim(), id: o.value }))
 ...
 */
export async function search(params, resolvedOffice) {
    const page = await getPage();
    await page.goto(SEARCH_PAGE, { waitUntil: "domcontentloaded" });
    if (!(await isLoggedIn(page)))
        throw new NotLoggedInError();
    ...
    if (params.dateFrom || params.dateTo) {
        ...
        if ((await rangeToggle.count()) > 0)
            await rangeToggle.first().check().catch(() => { });
        const dateInputs = form.getByPlaceholder("gg.aa.yyyy");
        if (params.dateFrom)
            await dateInputs.nth(0).fill(toIso(params.dateFrom));
        if (params.dateTo)
            await dateInputs.nth(1).fill(toIso(params.dateTo));
    }
...
export async function getIlanPdf(guid) {
