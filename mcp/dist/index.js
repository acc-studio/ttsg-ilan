...
import { z } from "zod";
import { login, loadOffices, search, getIlanPdf, close, NotLoggedInError, } from "./ttsg.js";
...
server.tool("ttsg_offices", "List the trade registry offices (Ticaret Sicili Müdürlükleri) available in the İlan Görüntüleme search, with their ids. Optionally filter by a name fragment. Requires an active session.", {
    filter: z
        .string()
        .optional()
...
server.tool("ttsg_search", "Search Türkiye Ticaret Sicili Gazetesi announcements. A registry office is REQUIRED (name or id), plus either a company name (unvan, min 5 chars) or a registry number (sicilNo). Returns the list of matching ilanlar with their Guid (use ttsg_get_ilan to fetch a PDF). Requires an active session.", {
    office: z
        .string()
    ...
    for (const [label, v] of [
        ...
        if (v && !DATE_RE.test(v))
            return errorResult(`'${label}' must be in dd.mm.yyyy format.`);
    }
    try {
        const offices = await loadOffices();
        ...
        const result = await search({ office: resolved.id, unvan, sicilNo, dateFrom, dateTo, includeCourt }, resolved);
        if (result.count === 0) {
            return textResult(`No announcements found in ${resolved.name} for ${unvan ? `unvan "${unvan}"` : `sicil no ${sicilNo}`}.`);
        }
        const header = `${result.count} announcement(s) in ${resolved.name}:`;
        const body = result.ilanlar
            .map((i, n) => `${n + 1}. ${i.yayinTarihi} | Sayı ${i.sayi}${i.sayfa ? `, Sayfa ${i.sayfa}` : ""} | ${i.ilanTuru}\n   ${i.unvan} (Sicil ${i.sicilNo})\n   Guid: ${i.guid}`)
            .join("\n");
...
server.tool("ttsg_get_ilan", "Download a gazette announcement PDF by its Guid (from ttsg_search) through the authenticated session. Returns the saved file path; read that file to view/summarize the ilan. Requires an active session.", {
    guid: z.string().describe("The ilan Guid returned by ttsg_search."),
}, async ({ guid }) => {
    try {
        const r = await getIlanPdf(guid);
        return textResult(`Saved ilan PDF (${r.bytes} bytes, ${r.contentType}) to:\n${r.filePath}\n\nRead this file to view or summarize the announcement.`);
    }
    catch (err) {
        if (err instanceof NotLoggedInError)
            return errorResult(err.message);
